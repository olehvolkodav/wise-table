    ...
    <>
      {
        ...
        <p className="text-center text-lg leading-5 font-bold">
          { `${getFormattedNumber(Number(cheapAmount.toFixed(2)))} ${currency?.currency}` }
        </p>
      }
      {
        ...
        <p className="text-center text-sm">
          ...
          { `${getFormattedNumber(Number((cheapAmount - expensiveAmount).toFixed(2)))} ${currency?.currency}` }
        </p>
      }
...
const ContentItem: React.FC<ContentItemProps> = memo(({ item, contextValue }) => {
  const { providers, currencies } = contextValue;
  
  const srcCurrencyItem = useMemo<CurrencyType | undefined>(() => {
    return currencies.find(currency => currency.countryCode === item.source)
  }, [currencies, item]);

  const tarCurrencyItem = useMemo<CurrencyType | undefined>(() => {
    return currencies.find(currency => currency.countryCode === item.target)
  }, [currencies, item]);

  const cheapProvider = useMemo<ProviderType | undefined>(() => {
    if(!item.cheapProvider) return undefined;
    return providers.find(provider => provider.id === item.cheapProvider?.id);
  }, [providers, item]);

  return (
    <tr>
      <td className="p-4">
        { srcCurrencyItem?.country || "" }
      </td>
      <td className="p-4">
        { tarCurrencyItem?.country || "" }
      </td>
      <td className="p-4">
        {
          cheapProvider?.logo ?
      ...
      <td className="p-4 text-center">
        { `${getFormattedNumber(item.amount)} ${tarCurrencyItem?.currency || ""}` }
      </td>
      <td className="p-4">
        <RecipientColumn
